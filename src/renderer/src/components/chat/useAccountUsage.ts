 ...
 *
 * Every pane calls this and panes on the same provider get the same answer:
 * main caches each provider for a minute and collapses concurrent asks into
 * one request. That is why this is a plain hook rather than a store threaded
 * down from App — there is nothing to coordinate.
 *
...
export function useAccountUsage(
  ...
  useEffect(() => {
    let cancelled = false
    // Never carry one provider's percentage across the selector changing.
    setUsage(null)
    const read = (force: boolean): void => {
      void window.api
        .accountUsage(source, force)
        .then((next) => {
